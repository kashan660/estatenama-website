...
const db = {
    ...
    async updatePage(id, page) {
        ...
        for (const [key, col] of Object.entries(mappings)) {
            if (page[key] !== undefined) { fields.push(`${col} = $${i++}`); values.push(page[key]); }
        }
        if (page.sortOrder !== undefined) { fields.push(`sort_order = $${i++}`); values.push(page.sortOrder); }
        if (page.showInNav !== undefined) { fields.push(`show_in_nav = $${i++}`); values.push(!!page.showInNav); }
